...
┗━⬣ 𝐬𝐞𝐰𝐚𝐛𝐨𝐭
 ...
 ♥️Skyy Bot @2021`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
⬣  ${prefix}jadwalsholat
 ...
 ♥️Skyy Bot @2021`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`ikyygroup`,`sewabot`]
...
𝔍𝔞𝔫𝔤𝔞𝔫 𝔭𝔢𝔯𝔫𝔞𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔰𝔢𝔰𝔢𝔬𝔯𝔞𝔫𝔤 
 ...
 ♥️Skyy Bot @2021`, list)
   ...
   ///ISLAMI MENU  
case 'listsurah':
		 get_result = await fetchJson(`http://api.lolhuman.xyz/api/quran?apikey=skybotkey`)
		 ...
		 if (args.length < 1) return reply(`Example: ${prefix + command} 18 or ${prefix + command} 18/10 or ${prefix + command} 18/1-10`)
                    urls = `http://api.lolhuman.xyz/api/quran/${args[0]}?apikey=skybotkey`
		   ...
		   if (args.length == 0) return reply(`Example: ${prefix + command} 18 or ${prefix + command} 18/10`)
                    ...
                    break
                case 'asmaulhusna':
		 get_result = await fetchJson(`http://api.lolhuman.xyz/api/asmaulhusna?apikey=skybotkey`)
                    get_result = get_result.result
		   ...
		   if (args.length == 0) return reply(`Example: ${prefix + command} Muhammad`)
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/kisahnabi/${query}?apikey=skybotkey`)
                    get_result = get_result.result
		   ...
		   if (args.length == 0) return reply(`Example: ${prefix + command} Yogyakarta`)
                    daerah = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/sholat/${daerah}?apikey=skybotkey`)
                    get_result = get_result.result
...
//------------------< Premium >-------------------
       ...
       case 'sewabot':
              ...
              gopeynya = 'https://i.ibb.co/BLnrL6Y/shareqr.jpg'

              teksnya = `*── 「 PRICE LIST HARGA SEWA BOT」 ──*
...
ikyy.sendMessage(from,{url:'./'+memek},audio,{mimetype:'audio/mpeg'})
  ...
  case 'xnxxsearch':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/xnxxsearch?apikey=skybotkey&query=${query}`)
                    get_result = get_result.result
                    ini_txt = ""
                ...
                case 'xnxx':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/xnxx?apikey=skybotkey&url=${query}`)
                    get_result = get_result.result
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'sc': 
       case 'src':
              textImg(`Hububgi Owner jika ingin SC nya :) `)
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
